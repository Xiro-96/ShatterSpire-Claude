...
namespace Shatterspire
{
    ...
    public static class Loc
    {
        ...
        private static readonly Dictionary<string, string> German = new()
        {
            ...
            // ── Heldenstufen ────────────────────────────────────────────
            { "HERO LEVEL", "HELDENSTUFE" },
            { "TO NEXT LEVEL", "BIS ZUR NÄCHSTEN STUFE" },
            { "HIGHEST HERO LEVEL", "HÖCHSTE HELDENSTUFE" },
            { "XP", "EP" },
            { "NOVICE", "NEULING" },
            { "ADEPT", "GESELLE" },
            { "VETERAN", "VETERAN" },
            ...
            { "Survive one lethal hit per climb", "Einmal je Aufstieg einen tödlichen Treffer überleben" },
            { "SPLINTER BURST", "SPLITTERBERSTEN" },
            { "Fallen enemies burst and hurt others", "Gefallene Gegner bersten und verletzen andere" },
            { "ADRENALINE", "ADRENALIN" },
            { "Nearby kills: +25% speed for 2 s", "Abschüsse in der Nähe: 2 s lang +25 % Tempo" },
            { "BLOOD PACT", "BLUTPAKT" },
            { "+25% damage, -25% maximum health", "+25 % Schaden, -25 % maximales Leben" },
            { "STORM BELL", "STURMGLOCKE" },
            { "Every 5th light attack calls lightning", "Jeder 5. leichte Angriff ruft einen Blitz" },
            { "SPARK WARD", "FUNKENSCHILD" },
            { "Dashing blocks the next hit", "Ein Dash blockt den nächsten Treffer" },
            { "PHANTOM EDGE", "PHANTOMKLINGE" },
            { "Your dash cuts enemies in its path", "Dein Dash schneidet Gegner auf dem Weg" },
            { "STEADY HEART", "RUHIGES HERZ" },
            { "Perfect heavy attacks heal 8%", "Perfekte Schwerangriffe heilen 8 %" },
            { "OVERFLOW", "ÜBERFLUSS" },
            { "Your skill fills half the heavy meter", "Die Fähigkeit füllt den Schwerbalken halb" },
            { "WAR BANNER", "KRIEGSBANNER" },
            { "+5% damage per floor, up to +40%", "+5 % Schaden je Etage, bis +40 %" },

            // ── Pause ──
            { "PAUSED", "PAUSE" },
            { "THE CLIMB WAITS FOR YOU", "DER AUFSTIEG WARTET AUF DICH" },
            { "RESUME", "WEITER" },
            { "LEAVE THE CLIMB?", "AUFSTIEG VERLASSEN?" },
            { "THE CLIMB ENDS AS A DEFEAT", "DER AUFSTIEG ENDET ALS NIEDERLAGE" },
            { "YOU KEEP", "DU BEHÄLTST" },
            { "OF YOUR SHARDS", "DEINER SPLITTER" },
            { "BACK", "ZURÜCK" },
            { "LEAVE", "VERLASSEN" },
